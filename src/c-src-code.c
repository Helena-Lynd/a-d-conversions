...
int main (void) {
  ...
  for (;;) { /* do forever */
		...
		/* Get a character from the user. Only accepts characters 1-5 inclusive */
		UInt8 servoNumInp = '0';
		while (servoNumInp < '1' || servoNumInp > '5'){
			servoNumInp = GetChar();
		}
		
		/* Echo the input character and move the cursor to a new line */
		PutChar(servoNumInp);
		PutChar(0x0D);
		PutChar(0x0A);
		
		/* Convert ASCII code to binary equivalent and subtract one */
		servoNumInp -= 0x31;
		
		/* Access DAC0 table*/
		UInt16 dacVal = DAC0_table[servoNumInp];
		
		/* Set DAC0 output voltage to midpoint of desired segment of voltage range */
		DAC0->DAT[0].DATL = (UInt8) (DAC0_table [servoNumInp] & 0xFF);
		DAC0->DAT[0].DATH = (UInt8) (DAC0_table [servoNumInp] >> 8);
		...
		/* Write duty count value out */
		TPM0->CONTROLS[2].CnV = PWM_duty_table [servoNumInp];
